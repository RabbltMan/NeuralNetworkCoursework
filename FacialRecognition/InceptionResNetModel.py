...
class InceptionResNetModel(object):
    ...
    def __init__(self, lr: float = 0.0002, BATCH_SIZE: int = 5) -> None:
        ...
        self.lossFunc = TripletMarginLoss()
        ...
        for epoch in range(EPOCH):
            ...
            if not epoch % 5:
                ...
                self.testTripletDataset = TripletDataset(
                    self.posPathList, self.negPath, 325)
                self.testTripletDataLoader = DataLoader(
                    self.testTripletDataset, 75)
                trainLen = len(self.trainTripletDataLoader)
            runningLoss = 0
            self.model.train()
            for i, (A, P, N) in enumerate(self.trainTripletDataLoader):
                print(
            ...
            runningValLoss = 0
            with no_grad():
                for (valA, valP, valN) in self.testTripletDataLoader:
                    yValA = self.model(valA)
                    yValP = self.model(valP)
                    yValN = self.model(valN)
                    runningValLoss += self.lossFunc(yValA, yValP, yValN)
